...
<meta http-equiv="X-UA-Compatible" content="edge" />
 <c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/portalpayOrder/edit')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/portalpayOrder/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/portalpayOrder/view')}">
	<script type="text/javascript">
		$.canView = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/portalpayOrder/viewGoods')}"><!-- 商品按钮  点击显示商品详情 -->
	<script type="text/javascript">
		$.canGoodsView = true;
	</script>
</c:if>
...
<title>DEMO管理</title>
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/portalpay/portalpayOrder/dataGrid?type=cart',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 350,
			height : 600,
			href : '${ctx}/portalpay/portalpayOrder/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
			if (b) {
				progressLoad();
				$.post('${ctx}/portalpay/portalpayOrder/delete', {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 400,
			href : '${ctx}/portalpay/portalpayOrder/editPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
	...
	function editLogisticsFun(id,orderNumber) {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 400,
			href : '${ctx}/portalpay/portalpayOrder/editLogisticsPage?orderNumber=' + orderNumber+'&orderId='+id,// 商品订单号和商品id  orderId用来当物流信息更新时，订单中的物流单号
			buttons : [ {
				text : '编辑',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function viewFun(orderNumber) {
		...
		parent.$.modalDialog({
			...
			width : 800,//详情也面宽度
			height : 600,//详情页面高度
			href : '${ctx}/portalpay/portalpayOrder/viewPage?orderNumber=' + orderNumber
		});
	}
	...
	function viewGoodsFun(orderNumber) {
		...
		parent.$.modalDialog({
			...
			width : 950,//详情也面宽度
			height : 400,//详情页面高度
			href : '${ctx}/portalpay/portalpayOrder/viewGoodsPage?orderNumber=' + orderNumber
		});
	}
	function searchFun() {
	...
	function exportFun(){
		//progressLoad();
		$.post('${ctx}/portalpay/portalpayOrder/export', {
			//id : id
	...
	function dialogHandle(){
		var content=$('#dlgContent').val();
		  $.post('${ctx}/portalpay/portalpayOrder/decrypt', {
			  content : content
			}, function(result) {
				if (result.success) {
					$('#dlgContentD').val(result.msg)
				}else{
...
<body class="easyui-layout" data-options="fit:true,border:false"  onload="initDateTime()">
	...
	<div id="toolbar" style="display: none;">
		...
		<jsp:include page="decryptPortal.jsp"></jsp:include>
				...
				<table>
					<tr>
					   ...
					   <select class="easyui-combobox"  label="State:" labelPosition="top" style="width:100px;" name='payStatus' id='payStatus'>
						...
						<a href="javascript:void(0)" class="easyui-linkbutton" onclick="$('#dlg').dialog('open')" style="height: 20px;color: #76C27A" >解密订单</a>
						</td>
					</tr>
				</table>
			</form>
