...
<meta http-equiv="X-UA-Compatible" content="edge" />
 <%-- <c:if test="${fn:contains(sessionInfo.resourceList, '/jinxiutong/ordersInfo/edit')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if> --%>
<c:if test="${fn:contains(sessionInfo.resourceList, '/jinxiutong/ordersInfo/view')}">
	<script type="text/javascript">
		$.canView = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/jinxiutong/ordersInfo/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/jinxiutong/ordersInfo/cancelOrder')}">
	<script type="text/javascript">
		$.canCancelOrder = true;
	</script>
</c:if>
...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/jinxiutong/ordersInfo/dataGrid',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
	...
	function cancelOrder(orderNumber,id){
		...
		parent.$.messager.confirm('询问', '您是否要取消订单？', function(b) {
			if (b) {
				progressLoad();
				$.post('${ctx}/jinxiutong/ordersInfo/cancelOrder', {
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
			if (b) {
				progressLoad();
				$.post('${ctx}/jinxiutong/ordersInfo/delete', {
					id : id
				}, function(result) {
					if (result.success) {
						parent.$.messager.alert('提示', result.msg, 'info');
	...
	function viewFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 900,//详情也面宽度
			height : 600,//详情页面高度
			href : '${ctx}//jinxiutong/ordersInfo/viewPage?id=' + id
		});
	}
	...
	function viewGoodsFun(orderNumber) {
		...
		parent.$.modalDialog({
			...
			width : 950,//详情也面宽度
			height : 400,//详情页面高度
			href : '${ctx}/jinxiutong/ordersInfo/viewGoodsPage?orderNumber=' + orderNumber
		});
	}
	</script>
</head>
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" style="display: none;">
		<div>
			<%-- <c:if test="${fn:contains(sessionInfo.resourceList, '/jinxiutong/ordersInfo/add')}">
				 <a onclick="addFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">添加</a>
			</c:if>
			<c:if test="${fn:contains(sessionInfo.resourceList, '/jinxiutong/ordersInfo/informationCart')}">
			...
			<form id="searchForm">
				<table>
					<tr>
						...
						<th>买家电话:</th>
						<td><input name="tele" id="tele" placeholder="请输入买家电话"/></td>
						<th>创建时间:</th>
						<td>
