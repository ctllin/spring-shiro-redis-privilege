...
<meta http-equiv="X-UA-Compatible" content="edge" />
 <c:if test="${fn:contains(sessionInfo.resourceList, '/alipay/refundRecordZfb/edit')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/alipay/refundRecordZfb/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/alipay/refundRecordZfb/view')}">
	<script type="text/javascript">
		$.canView = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/alipay/refundRecordZfb/viewGoods')}"><!-- 商品按钮  点击显示商品详情 -->
	<script type="text/javascript">
		$.canGoodsView = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/alipay/refundRecordZfb/logisticsEdit')}"><!-- 物流按钮  点击显示物流详情 -->
	<script type="text/javascript">
...
<title>DEMO管理</title>
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/alipay/refundRecordZfb/dataGrid',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 350,
			height : 600,
			href : '${ctx}/alipay/refundRecordZfb/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
					var f = parent.$.modalDialog.handler.find('#addFormId');//对应refundRecordZfbAdd.jsp的from  id
					f.submit();
				}
			} ]
		});
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
			if (b) {
				progressLoad();
				$.post('${ctx}/alipay/refundRecordZfb/delete', {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 400,
			href : '${ctx}/alipay/refundRecordZfb/editPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
					var f = parent.$.modalDialog.handler.find('#editForm');//对应refundRecordZfbEdit.jsp的from  id
					f.submit();
				}
			} ]
		});
	...
	function editLogisticsFun(id,orderNumber) {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 400,
			href : '${ctx}/alipay/refundRecordZfb/editLogisticsPage?orderNumber=' + orderNumber,
			buttons : [ {
				text : '编辑',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function viewFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 800,//详情也面宽度
			height : 600,//详情页面高度
			href : '${ctx}/alipay/refundRecordZfb/viewPage?id=' + id
		});
	}
	...
	function viewGoodsFun(outTradeNo) {
		...
		parent.$.modalDialog({
			...
			width : 950,//详情也面宽度
			height : 400,//详情页面高度
			href : '${ctx}/alipay/refundRecordZfb/viewGoodsPage?outTradeNo=' + outTradeNo
		});
	}
	...
	function refundFun(){
		parent.$.modalDialog({
			...
			width : 800,
			height : 400,
			href : '${ctx}/alipay/refundRecordZfb/refund',
			buttons : [ {
				text : '退款',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
					var f = parent.$.modalDialog.handler.find('#refundForm');//对应tradeSeatorderZfbEdit.jsp的from  id
					f.submit();
				}
			} ]
		});
...
<body class="easyui-layout" data-options="fit:true,border:false"  onload="initDateTime()">
	...
	<div id="toolbar" style="display: none;">
		<div>
			<c:if test="${fn:contains(sessionInfo.resourceList, '/alipay/refundRecordZfb/add')}">
				 <a onclick="addFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">添加</a>
			</c:if>
			<c:if test="${fn:contains(sessionInfo.resourceList, '/alipay/refundRecordZfb/refund')}">
				<a onclick="refundFun();" href="javascript:void(0);" class="easyui-linkbutton" style="height: 20px">退款</a>
			</c:if>
		</div>
