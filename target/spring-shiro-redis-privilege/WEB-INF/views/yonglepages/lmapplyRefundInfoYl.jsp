...
<meta http-equiv="X-UA-Compatible" content="edge" />
 <c:if test="${fn:contains(sessionInfo.resourceList, '/yongle/lmapplyRefundInfoYl/edit')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/yongle/lmapplyRefundInfoYl/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/yongle/lmapplyRefundInfoYl/view')}">
	<script type="text/javascript">
		$.canView = true;
	</script>
</c:if>
<title>DEMO管理</title>
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/yongle/lmapplyRefundInfoYl/dataGrid',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
			...
			columns : [ [ /* {
				...
				title : 'status',
				field : 'status',
				//处理状态 0代表接受，1代表退款已经提交2退款成功3退款失败
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 350,
			height : 600,
			href : '${ctx}/yongle/lmapplyRefundInfoYl/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
			if (b) {
				progressLoad();
				$.post('${ctx}/yongle/lmapplyRefundInfoYl/delete', {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 250,
			height : 200,
			href : '${ctx}/yongle/lmapplyRefundInfoYl/editPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
	...
	function viewFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 800,//详情也面宽度
			height : 400,//详情页面高度
			href : '${ctx}/yongle/lmapplyRefundInfoYl/viewPage?id=' + id
		});
	}
	function searchFun() {
	...
	function exportFun(){
		//progressLoad();
		$.post('${ctx}/yongle/lmapplyRefundInfoYl/export', {
			//id : id
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" style="display: none;">
		<div>
			<c:if test="${fn:contains(sessionInfo.resourceList, '/yongle/lmapplyRefundInfoYl/add')}">
				 <a onclick="addFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">添加</a>
			</c:if>
			<c:if test="${fn:contains(sessionInfo.resourceList, '/yongle/lmapplyRefundInfoYl/export')}">
