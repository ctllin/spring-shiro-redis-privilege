...
<meta http-equiv="X-UA-Compatible" content="edge" />
 <c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/portalpayOrderSdy/edit')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/portalpayOrderSdy/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/portalpayOrderSdy/view')}">
	<script type="text/javascript">
		$.canView = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/portalpayOrderSdy/viewGoods')}"><!-- 商品按钮  点击显示商品详情 -->
	<script type="text/javascript">
		$.canGoodsView = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/portalpayOrderSdy/logisticsEdit')}"><!-- 物流按钮  点击显示物流详情 -->
	<script type="text/javascript">
...
<title>DEMO管理</title>
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 350,
			height : 600,
			href : '${ctx}/portalpay/portalpayOrderSdy/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
			if (b) {
				progressLoad();
				$.post('${ctx}/portalpay/portalpayOrderSdy/delete', {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 400,
			href : '${ctx}/portalpay/portalpayOrderSdy/editPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
	...
	function editLogisticsFun(id,orderNumber) {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 400,
			href : '${ctx}/portalpay/portalpayOrderSdy/editLogisticsPage?orderNumber=' + orderNumber+'&orderId='+id,// 商品订单号和商品id  orderId用来当物流信息更新时，订单中的物流单号
			buttons : [ {
				text : '编辑',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function exportFun(){
		//progressLoad();
		$.post('${ctx}/portalpay/portalpayOrderSdy/export', {
			//id : id
...
<body class="easyui-layout" data-options="fit:true,border:false" onload="initDateTime()">
	...
	<div id="toolbar" style="display: none;">
		<div>
			<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/portalpayOrderSdy/add')}">
				 <a onclick="addFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">添加</a>
			</c:if>
			<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/portalpayOrderSdy/export')}">
