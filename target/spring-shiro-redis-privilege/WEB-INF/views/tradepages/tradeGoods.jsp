...
<meta http-equiv="X-UA-Compatible" content="edge" />
 <c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/tradeGoods/edit')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/tradeGoods/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/tradeGoods/view')}">
	<script type="text/javascript">
		$.canView = true;
	</script>
</c:if>
<title>DEMO管理</title>
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/portalpay/tradeGoods/dataGrid',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 350,
			height : 400,
			href : '${ctx}/portalpay/tradeGoods/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
			if (b) {
				progressLoad();
				$.post('${ctx}/portalpay/tradeGoods/delete', {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 250,
			height : 200,
			href : '${ctx}/portalpay/tradeGoods/editPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
	...
	function viewFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 800,//详情也面宽度
			height : 400,//详情页面高度
			href : '${ctx}/portalpay/tradeGoods/viewPage?id=' + id
		});
	}
	function searchFun() {
	...
	function exportFun(){
		//progressLoad();
		$.post('${ctx}/portalpay/tradeGoods/export', {
			//id : id
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" style="display: none;">
		<div>
			<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/tradeGoods/add')}">
				 <a onclick="addFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">添加</a>
			</c:if>
			<c:if test="${fn:contains(sessionInfo.resourceList, '/portalpay/tradeGoods/export')}">
			...
			<form id="searchForm">
				<table>
					<tr>
						...
						<th>商品名称:</th>
						<td><input name="goodsName" id="goodsName" placeholder="请输入商品名称"/></td>
						<th>创建时间:</th>
						<td>
